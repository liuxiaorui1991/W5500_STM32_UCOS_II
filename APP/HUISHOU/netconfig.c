...
#define SYSTEMTICK_PERIOD_10MS  100 //系统的节拍周期(单位:0.1ms) ，为了这个时间单位不惜更改lwip的源码宏
#define INPUT_TMR_INTERVAL 	  50 //暂规定每5ms轮询一次以太网芯片(单位：0.1ms)
...
void LwIP_Init( void )
{
  ...
  gw.addr = 0; 
#else										//
  IP4_ADDR(&ipaddr, 10, 0, 0, 182);  		//设置网络接口的ip地址
  IP4_ADDR(&netmask, 255, 255, 255, 0);		//子网掩码
  IP4_ADDR(&gw, 10, 0, 0, 1);			    //网关
...
{
  LocalTime += SYSTEMTICK_PERIOD_10MS;
}
...
void LwIP_Periodic_Handle(__IO uint32_t localtime)
{
	...
	if(localtime - INPUT_Timer >= INPUT_TMR_INTERVAL)
	  ...
	  if (localtime - TCPTimer >= TCP_TMR_INTERVAL)
	  {
	    TCPTimer =  localtime;
	    tcp_tmr();		 //每250ms调用一次
			OSTimeDlyHMSM(0,0,0,30);//空出30ms给modbus_task查表
	  }
